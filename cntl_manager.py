...
def readCntlFile(cntlPath, sheet, account):
    import numpy as np
    import pandas as pd
    import os
    import itertools
    ...
    df=pd.read_excel(cntlPath, sheetname=sheet)
    ...
    simNames = df['variable name (If blank, all variables are obtained)']
    ind = pd.notnull(simNames)
    simNames = simNames[ind].tolist()
